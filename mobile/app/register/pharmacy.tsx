...
export default function RegisterPharmacy() {
  const [form, setForm] = useState({ name: '', email: '', healthRegNumber: '', phoneNumber: '', address: '', city: '', description: '', sectorType: '' });
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f8fafc' }}>
      ...
      <View style={{ backgroundColor: '#dc2626', paddingHorizontal: 20, paddingTop: 16, paddingBottom: 24, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Text style={{ color: 'white', fontSize: 18, fontWeight: '900' }}>تسجيل صيدلية</Text>
        <View style={{ width: 36 }} />
      </View>
      <ScrollView style={{ flex: 1, paddingHorizontal: 20, paddingTop: 24 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={{ marginBottom: 16 }}><Text style={sl}>اسم الصيدلية *</Text><TextInput style={si} placeholder="صيدلية..." placeholderTextColor="#94a3b8" value={form.name} onChangeText={set('name')} textAlign="right" /></View>
        <View style={{ marginBottom: 16 }}><Text style={sl}>البريد الإلكتروني *</Text><TextInput style={si} placeholder="pharmacy@email.com" placeholderTextColor="#94a3b8" value={form.email} onChangeText={set('email')} keyboardType="email-address" autoCapitalize="none" textAlign="right" /></View>
        ...
        <View style={{ marginBottom: 16 }}>
          ...
          <View style={{ flexDirection: 'row' }}>
            {SECTOR_TYPES.map((s, i) => (
              <TouchableOpacity key={s.value} style={{ flex: 1, paddingVertical: 14, borderRadius: 16, alignItems: 'center', borderWidth: 2, marginLeft: i === 0 ? 6 : 0, borderColor: form.sectorType === s.value ? '#dc2626' : '#e2e8f0', backgroundColor: form.sectorType === s.value ? '#fee2e2' : 'white' }} onPress={() => set('sectorType')(s.value)}>
                <Text style={{ fontWeight: 'bold', color: form.sectorType === s.value ? '#dc2626' : '#64748b' }}>{s.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        <View style={{ marginBottom: 16 }}><Text style={sl}>الوصف</Text><TextInput style={[si, { height: 80, textAlignVertical: 'top' }]} placeholder="وصف مختصر عن الصيدلية وخدماتها..." placeholderTextColor="#94a3b8" value={form.description} onChangeText={set('description')} multiline textAlign="right" /></View>
        <TouchableOpacity style={{ backgroundColor: isLoading ? '#f87171' : '#dc2626', borderRadius: 16, paddingVertical: 16, alignItems: 'center', marginBottom: 40 }} onPress={handleSubmit} disabled={isLoading}>
